        ...
        @forelse($opdTimelines as $opdTimeline)
            <div class="timeline-item row" date-is="{{ date('jS M, Y', strtotime($opdTimeline->date)) }}">
                <div class="col-md-9">
                    ...
                    <p>{!! !empty($opdTimeline->description) ? nl2br(e($opdTimeline->description)) : __('messages.common.n/a') !!}
                    </p>
                </div>
                <div class="text-end bottom-sm mb-5 col-md-3">
                    @if ($opdTimeline->opd_timeline_document_url != '')
                        <a data-turbo="false" title="{{ __('messages.document.download') }}"
                            class="btn px-1 text-info fs-2"
                            href="{{ url('opd-timelines-download' . '/' . $opdTimeline->id) }}" target="_blank">
